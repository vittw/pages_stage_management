...
(function($, undefined) {
	...
	var // Stream object instances
		...
		//ywh 2014/3/31 add XDomainRequest,fix ie8 ie9
		objxdr=null,
						...
						(function post() {
							if (stream.readyState === 1 && dataQueue.length) {
								...
								if (handleSend("send", options, stream) !== false) {
									//IE8、9下使用XDR
									if(stream.options.enableXDR && window.XDomainRequest){
										data.xdr="xdr";
										transport.extendOpen(tmpData,post);
									}else{
				...
				close: function() {
					...
					if (stream.readyState < 2) {
						...
						if (handleSend("close", options, stream) !== false) {
							//IE8、9下使用XDR
							if(stream.options.enableXDR && window.XDomainRequest){
								transport.extendOpen({"xdr":"xdr"},function(){});
							}else{
								// Notifies the server
								$.ajax(options);
							}
	...
	$.extend(transports, {
		...
		xdr: function(stream, handler) {
			...
			return {
				...
				extendOpen:function(data,post){
				    if($.browser.version != '8.0' || stream.options.closeFlag==0){
				        objxdr=new XDomainRequest();
					    objxdr.onload=post;
					    objxdr.open("GET",prepareURL(stream.url, data));
					    objxdr.send();
					}else{
					    xdr.onload=post;
					    try{xdr.open("GET",prepareURL(stream.url, data));}catch(e){}
					    try{xdr.send();}catch(e){}
					    //setTimeout(function(){xdr.onload=onload;},300);
					}
				}
			};
		}
