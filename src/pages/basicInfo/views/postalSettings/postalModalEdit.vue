<template>
  <Form :model="formData" ref="formData" :rules="validate">
    ...
    <Row :gutter="32">
      <Col span="12">
        <FormItem label="是否启用" label-position="top">
...
<script>
import { getProvince, editPostalData } from '@/api/baseInfo/postalSettings';
...
export default {
  ...
  data () {
    return {
      userState: {
        loading: false,
        options: [{Key:this.editData.userCode,Value:this.editData.userName}]
      },
      provinceOptions: [],
      formData: {
        provinceData: {value:this.editData.provinceCode,label:this.editData.province},   //省份对象 包含Id 和 name
        provinceCode: this.editData.provinceCode,                                            //省份编号
        userData: {},                                                                    //用户对象 包含Id 和 name
        userCode: '',                                                                    //用户编号
        remark: this.editData.remark,
        isOpen : this.editData.deletionStateCode == 0 ? '启用':'禁用'
      },
      validate: {
				provinceCode:[
