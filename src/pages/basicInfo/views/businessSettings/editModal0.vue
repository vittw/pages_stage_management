<template>
  <Form :label-width="110" label-position="left">
    <Row :gutter="48">
      <Col span="10" class="divider_left">
        <p class="title_header">基础信息</p>
        <div class="demo-drawer-profile">
          <Row>
            <Col span="24">
              <FormItem label="工单来源">
                <Select v-model="detailData.dataSourceId" placeholder="请选择" :transfer="true">
          ...
          <Row>
            <Col span="24">
              <FormItem label="工单编号（外部）">
                <Input v-model="detailData.code" placeholder="请输入" />
              </FormItem>
            </Col>
          </Row>
          <Row>
            <Col span="24">
              <FormItem label="工单大类（外部）">
                <Input v-model="detailData.bigType" placeholder="请输入" />
              </FormItem>
            </Col>
          </Row>
          <Row>
            <Col span="24">
              <FormItem label="工单小类（外部）">
                <Input v-model="detailData.smallType" placeholder="请输入" />
              </FormItem>
            </Col>
          </Row>
          <Row>
            <Col span="24">
              <FormItem label="工单类型" prop="typeName">
                <Cascader v-model="typeName" :data="sourceTypeData" placeholder="请选择工单类型" :transfer="true"></Cascader>
              </FormItem>
            </Col>
          </Row>
          <Row>
            <Col span="24">
              <FormItem label="创建时间">
                <Input v-model="detailData.createOn" placeholder="请输入" />
              </FormItem>
            </Col>
          </Row>
          <Row>
            <Col span="24">
              <FormItem label="创建人">
                <Input v-model="detailData.createBy" placeholder="请输入" />
              </FormItem>
            </Col>
          </Row>
        </div>
      </Col>
      <Col span="14" divider_right>
        ...
        <div class="demo-drawer-profile">
          <Row :gutter="32">
            ...
            <Col span="12">
              <FormItem label="是否网点处理">
                <Select v-model="detailData.isSiteProcess" placeholder="请选择" :transfer="true">
                  <Option :value="0">是</Option>
                  <Option :value="1">否</Option>
                </Select>
              </FormItem>
          ...
          <Row :gutter="32">
            <Col span="12">
              <FormItem label="是否允许首跟">
                <Select v-model="detailData.isSmsNotify" placeholder="请选择" :transfer="true">
                  <Option :value="0">是</Option>
                  <Option :value="1">否</Option>
                </Select>
              </FormItem>
            ...
            <Col span="12">
              <FormItem label="首跟回复时间">
                  <Input v-model="detailData.siteSmsNotifyTime" placeholder="请输入" :number="true">
          ...
          <Row :gutter="32">
            <Col span="12">
              <FormItem label="网点是否超时罚款">
                <Select v-model="detailData.isSiteOvertime" placeholder="请选择" :transfer="true">
                  <Option :value="0">是</Option>
                  <Option :value="1">否</Option>
                </Select>
              </FormItem>
            </Col>
            <Col span="12" v-if="detailData.isSiteOvertime==0">
              <FormItem label="罚款金额">
          ...
          <div class="bg_gray">
            <Row  :gutter="20">
              <Col span="12">
                <FormItem label="时效类型">
                    <Select v-model="detailData.siteOvertimeType" placeholder="请选择" :transfer="true">
              ...
              <Col span="12" v-if="detailData.siteOvertimeType==0 || detailData.siteOvertimeType==1">
                <FormItem label="时效时间">
                    <Input v-model="detailData.siteOvertimeTime" placeholder="请输入" />
                </FormItem>
              </Col>
            </Row>
            <Row :gutter="20" v-if="detailData.siteOvertimeType==2">
              ...
              <Col span="7">
                <TimePicker type="time" v-model="detailData.siteMorningDealTime" placeholder="选择时间"></TimePicker>
              </Col>
            </Row>
            <Row :gutter="20" v-if="detailData.siteOvertimeType==2">
              <Col span="17">
              ...
              <Col span="7">
                <TimePicker type="time" v-model="detailData.siteAfternoonDealTime" placeholder="选择时间"></TimePicker>
              </Col>
            </Row>
          </div>
          <!-- <div class="bg_gray">
          ...
          <div class="bg_gray" v-if="detailData.isAssist==0">
            <Row :gutter="32">
              <Col span="12">
            ...
            <Row :gutter="32">
              <Col span="12">
                <FormItem label="可申请延迟次数">
                    <Input v-model="detailData.delayCount" placeholder="请输入" :number="true">
          ...
          <Row :gutter="32">
            <Col span="12">
              <FormItem label="是否允许申请完结">
                <Select v-model="detailData.isComplete" placeholder="请选择" :transfer="true">
                  <Option :value="0">是</Option>
                  <Option :value="1">否</Option>
                </Select>
              </FormItem>
            ...
            <Col span="12">
              <FormItem label="是否客户邀评">
                <Select v-model="detailData.isAssess" placeholder="请选择" :transfer="true">
                  <Option :value="0">是</Option>
                  <Option :value="1">否</Option>
                </Select>
              </FormItem>
...
export default {
  data(){
    return {
      sourceTypeData: [],
      typeName: [this.detailData.firstCode, this.detailData.secondCode, this.detailData.threeCode]
    }
  },
  props:{
    detailData:{
  ...
  methods:{
    handleSubmitEdit(){
      editBusinessConfig({
        ...
        isCompleteMessage: this.detailData.isCompleteMessage,     //申请完结时是否推送信息
        isSendMessageTips: this.detailData.isSendMessageTips,     //推送信息至客户时是否提示

        isUrge: this.detailData.isUrge,                //是否允许催办
        isAssist: this.detailData.isAssist,              //是否允许升级协助
        isDelay: this.detailData.isDelay,               //是否允许申请延迟
        delayCount: this.detailData.delayCount,              //可申请延迟次数
        delayTime: this.detailData.delayTime,             //延迟时间
        isComplete: this.detailData.isComplete,            //是否允许申请完结
        isAssess: this.detailData.isAssess,              //是否客户邀评

        //省区配置
        isProvinceProcess: this.detailData.isProvinceProcess,     //是否省区处理
        ...
        //二次申诉
        isSecondAppeal: this.detailData.isSecondAppeal,        //是否判断二次申诉
        secondFineMoney: this.detailData.secondFineMoney,         //二次申诉罚款金额
