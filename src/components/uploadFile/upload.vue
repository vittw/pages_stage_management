<template>
	<div class>
		<input name="file" type="file" id="file" class="files" @change="handleSuccess" 
					  accept="image/png,image/jpeg,.bmp,image/gif,.avi,.wma,.m4a,.wav,.rmvb,.rm,audio/mpeg,audio/mp4,audio/3gpp,
		...
		<div class="myFileBox">
			...
			<ul class="content" v-if="uploadList.length!=0">
              ...
              <img :src="item.browseUrl">
						...
						<Modal
							class-name="myImg"
							 v-model="imgModal" title="图片查看" class="myImg">
							<img :src="imgSurl">
						</Modal>
		...
		methods: {
			...
			addFile() {
				if(this.uploadList.length >= 5) {
					...
					let slen = this.$parent.model === undefined ? this.$parent.$parent.$parent.model.typeName.length : this.$parent.model.typeName.length
					if(slen >0) {
						document.getElementById("file").click();
					} else {
						this.$Message.info('工单类型为空时，不能上传附件');
					}
